  ...
  var adapter = {
    ls: function(dirname, cb) {
      MongoClient.connect(globalOpts.uri, {
        ...
        gfs.collection(globalOpts.bucket).ensureIndex({
          filename: 1,
          uploadDate: -1
          ...
          gfs.collection(globalOpts.bucket).distinct('filename', {
      ...
      MongoClient.connect(globalOpts.uri, {
        ...
        GridStore.exist(db, fd, globalOpts.bucket, function(err, exists) {
          ...
          gridStore.open(function(err, gridStore) {
            ...
            var decrypt = crypto.createDecipher(aes_algorithm, aes_password);
            stream.pipe(decrypt).pipe(concat(function(data) {
              return cb(null, data);
            }));
    ...
    readVersion: function(fd, version, cb) {
      MongoClient.connect(globalOpts.uri, {
        ...
        gfs.collection(globalOpts.bucket).ensureIndex({
          ...
          var cursor = gfs.collection(globalOpts.bucket).find({
            filename: fd
          });
          if (version < 0) {
            skip = Math.abs(version) - 1;
            cursor.limit(-1).skip(skip).sort([
              ['uploadDate', 'desc']
            ]);
          } else {
            cursor.limit(-1).skip(version).sort([
              ['uploadDate', 'asc']
            ]);
          }
    ...
    receive: function GridFSReceiver(options) {
      ...
      receiver__._write = function onFile(__newFile, encoding, done) {
        ...
        MongoClient.connect(globalOpts.uri, {
          ...
          var encrypt = crypto.createCipher(aes_algorithm, aes_password);
          __newFile.pipe(encrypt).pipe(outs);
        });
      };
      return receiver__;
    }
